    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="p-2 mt-2 text-sm text-red-400 bg-red-800 rounded-md">
                        {{ __('O seu endereço de e-mail não foi verificado.') }}

                        <button form="send-verification" class="text-sm text-blue-300 underline rounded-md hover:text-blue-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
